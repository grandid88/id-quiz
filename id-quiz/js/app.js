...
async function submitAnswer(answer) {
  ...
  try {
    ...
    // ✅ Plus de transaction sur quiz/scores ici —
    // le score par table est calculé en moyenne côté screen.js
  } catch (err) {
    console.error(err);
  }
}
...
async function showQuestionResult(qIdx) {
  ...
  // Calcul du score moyen cumulé de la table sur toutes les questions jouées
  let tableTotal = 0;
  for (let i = 0; i <= qIdx; i++) {
    const snap = await db.ref(`quiz/responses/q${i}/${sanitizeKey(player.table)}`).get();
    const tableData = snap.val() || {};
    const corrects = Object.values(tableData).filter(r => r.correct);
    if (corrects.length > 0) {
      const avgPts = Math.round(corrects.reduce((s, r) => s + (r.points || 0), 0) / corrects.length);
      tableTotal += avgPts;
    }
  }

  document.getElementById('result-table-score').textContent =
    `Table ${player.table} : ${tableTotal} pts (moyenne)`;
